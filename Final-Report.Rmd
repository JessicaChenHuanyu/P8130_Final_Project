...
building strong support to students' overall academic progress.
...
predictors for three subjects (**Figure 8**).

Based on the full models, we did some tests and calculations: First, we conducted the boxcox method (**Figure 6**) to determine if there was any transformation needed. Second, we calculated Cook's distance (**Figure 7**) to check the existence of outliers and influence points. Finally, in order to test the multicollinearity among predictors, we calculated VIF as the criterion of multicollinearity (**Table 6,7,8**).
...
remaining predictors, coefficients, and p-values are reported in **Table 3,4,5**. For BIC-based method, the remaining predictors and estimated coefficients are reported in **Table 9,10,11**. In the selection procedure using LASSO method, for each subject we used
cross-validation to decide the optimal value of method parameter
$\lambda$ (**Figure 9,10,11**), and then fitted LASSO model with this optimal value (**Figure 12,13,14; Table 12,13,14**).
...
predictors (**Table 15,16,17,18,19,20**). The VIFs are calculated for all
six models generated in this step to reveal the potential
multicollinearity (**Table 21,22,23,24,25,26**).
