...
```{r}
# math
math_sr = step(model_math_full, direction = 'both')
...
```{r}
# Math
math_1 = lm(math_score ~ . - reading_score, data = data) |>
  step(direction = "both")
math_1 |> summary()


math_2 = lm(math_score ~ . - writing_score, data = data) |>
  step(direction = "both")
math_2 |> summary()
...
# check VIF
vif_math_1 =
  performance::check_collinearity(math_1) |>
  as_tibble() |>
  mutate(VIF_CI = str_c("[", round(VIF_CI_low, 1), ", ", round(VIF_CI_high, 1), "]")) |>
  dplyr::select(Term, VIF, VIF_CI, Tolerance)
knitr::kable(x = vif_math_1, caption = "VIF for Math Score", digits = 1)

vif_math_2 =
  performance::check_collinearity(math_2) |>
  as_tibble() |>
  mutate(VIF_CI = str_c("[", round(VIF_CI_low, 1), ", ", round(VIF_CI_high, 1), "]")) |>
  dplyr::select(Term, VIF, VIF_CI, Tolerance)
knitr::kable(x = vif_math_2, caption = "VIF for Math Score", digits = 1)
