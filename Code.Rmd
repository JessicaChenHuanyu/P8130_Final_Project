...
step(model_math_full, direction = "both")
model_math_fit_both = lm(formula = math_score ~ gender + ethnic_group + parent_educ + 
    lunch_type + test_prep + parent_marital_status + practice_sport + 
    is_first_child + wkly_study_hours, data = data)
summary(model_math_fit_both)
```
...
set.seed(1)
cv_object = cv.glmnet(as.matrix(data[1:11]), data$math_score,
                         lambda = lambda_seq,
                         nfolds = 5)
cv_object$lambda.min

model_math_lasso = glmnet(as.matrix(data[1:11]), data$math_score, lambda = cv_object$lambda.min)
